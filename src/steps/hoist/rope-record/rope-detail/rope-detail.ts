...
When('I provide the following information in the Breaking load form', async function (this: IHoistWorld, breakingLoadInfo: DataTable) {
...
Then('I should be able to see the updated Breaking load data', async function (this: IHoistWorld) {
  ...
  const valueLocator = this.page?.locator('div').filter({ hasText: new RegExp(`^${expectedValue}$`) });
